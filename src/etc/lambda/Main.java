...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("====================================================");
        // 녹색이면서 100g 이상인 사과만 필터
        // 인터페이스를 구현하는 익명 클래스를 람다식으로 선언.
        // 인터페이스의 추상메서드가 단 하나일 경우에만 가능합니다.
        // 구현하고자 하는 추상 메서드 내부 로직이 한 줄일 경우에는 중괄호와 리턴을 생략할수있다.
        //어차피 filterApple 의 두번째 매개값은 무조건 ApplePredicate 타입의 객체가 전달되어야하는거고
        //ApplePredicate 는 추상메서드가 하나니까 굳이 메서드 이름을 쓸 필요가없는거자
        //게다가 메서드 내부에는 return 한줄만 쓸거라서 괄호랑 return 을 생략해 버리니까 이런모양이나온다
        ...
        System.out.println("========================================");
        //색깔이 빨강 혹은 초록이면서 무게가 150g 미만이여야한다
        ...
        //람다식에서 구현하는 메서드의 매개변수가 하나일경우에는 소괄호,리턴 생략 가능.
        //매개변수가없거나 , 두개이상일때는 소괄호,리턴 꼭 붙여야합니다
        List<Integer> filter1 = filter(numbers, (n) -> n % 2 == 1);
        System.out.println("filter1 = " + filter1 );


        List<String> foods = List.of("짜장면", "탕수육", "짬뽕", "피자", "삼겹살");
       List<String> food3 = filter(foods , (f) -> f.length() == 3);
        ...
        List<Color> colorsList = MappingApple.mappingAppleByColor(appleBasket);
        System.out.println("colorList = " + colorsList);
