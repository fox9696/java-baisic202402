...
public class MyBirth {
    ...
    -외부에서 직접 필드에 접근할수없도록 필드에 private 제한을 붙임 ,
 ...
 /*
    ...
    public void setMonth(int month) {
        if (month > 12 || month < 1) {
            System.out.println("잘못된 입력입니다 1~12월중에입력하세요");
            return;
        }
        this.month = month;
    }
    ...
    public void setDay(int day) {
        if (day > 31 || day < 1 ) {
            System.out.println("잘못된 입력입니다 1~31일중에입력하세요");
            return;
        }else {  // day 가 1~31일까지 들어옴
            if (this.month == 0) {
                System.out.println("일자를 할당하기 위해서 월 세팅이 선행되어야합니다.");
            return;
            }


            if (!isValidateMonth(day)) {
                System.out.println("월에 따른 일자가 올바르지않습니다");
           return;
            }
            this.day = day;
        }
...
//메서드가 private이다 ? -> 해당 클래스 내에서만 호출한다 외부에서는 아예 접근x
    ...
    public  void birthInfo() {
        //year , month , day 중 단 하나라도 제대로 값이 세팅되지 않았다면
        //출력을 해 주지 않겠다 .
        if (this.year ==0 || this.month ==0 || this.day ==0 ) {
            System.out.println("날짜 필드 중에 초기화되지않은 데이터가 있습니다");
            return;
        }
