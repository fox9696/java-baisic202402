...
public class FieldAndLocal {
    ...
    void printNumber(int c) {      // int c는 매개변수이고 printerNumber 를 불러야만 쓸수잇어서 지역변수임
        int b = 10;  //지역변수 -> 반드시초기화기를 수동으로해야함
    ...
    void foo() {
        System.out.println("필드 a = " + a);  //필드는 어느 메서드에서든지 참조가가능
    //    System.out.println("지역변수 b = " + b);  *얘는 printNumber 안에 지역변수라 여기선호출안됨
    }
